...
const TextButton: React.FC<ButtonProps> = ({
  ...
  const renderLeadIcon = (color: string) => {
    ...
    return LeadIcon && <LeadIcon width={iconSize} height={iconSize} outlineColor={color} fillColor={colors.transparent} />;
  };
  ...
  const renderTrailIcon = (color: string) => {
    ...
    if (size === MEDIUM) iconSize = BUTTON_ICON_SIZE_MEDIUM;
    if (size === LARGE) iconSize = BUTTON_ICON_SIZE_LARGE;
    return TrailIcon && <TrailIcon width={iconSize} height={iconSize} outlineColor={color} fillColor={colors.transparent} />;
  };
